document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll("button[data-course-id]").forEach((button) => {
    button.addEventListener("click", function () {
      ...
      fetch(
        `index.php?router=course&action=getDisciplines&courseId=${courseId}`
      )
        .then((response) => response.json())
        .then((data) => {
          // Limpa a tabela antes de adicionar os novos dados
          disciplineTableBody.innerHTML = "";
          // Verifica se o array de disciplinas está vazio
          if (data.length === 0) {
            // Se não houver disciplinas, exibe uma mensagem informando
            const messageRow = document.createElement("tr");
            messageRow.innerHTML = `
             <td colspan="4" style="text-align: center;">No disciplines available in this course.</td>
             `;
            disciplineTableBody.appendChild(messageRow);
          } else {
