<div class="sidebar">
    <h3>Menu</h3>
    <a href="?router=professor" class="quickLinks__card__router"><i class="fa-solid fa-chalkboard-user"></i> Professors</a>
    <a href="?router=course" class="quickLinks__card__router"><i class="fa-solid fa-scroll"></i> Courses</a>
    <a href="?router=discipline" class="quickLinks__card__router"><i class="fa-solid fa-book"></i> Disciplines</a>
    <a href="?router=student" class="quickLinks__card__router"><i class="fa-solid fa-graduation-cap"></i> Students</a>
    <a href="?router=hub" class="quickLinks__card__router"><i class="fa-solid fa-building-flag"></i> Hubs</a>
    <a href="?router=user" class="quickLinks__card__router"><i class="fa-solid fa-user"></i> Users</a>
</div>